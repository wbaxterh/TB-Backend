...
module.exports = (db) => {
  ...
  router.get('/:id/stats', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          'DEBUG Stats - sample post userId:',
          samplePost?.userId,
          'type:',
          typeof samplePost?.userId,
        );

        // Try both string and ObjectId formats
        const countByString = await feedPostsCollection.countDocuments({ userId: id });
        const countByObjectId = await feedPostsCollection.countDocuments({
          userId: new ObjectId(id),
        });
        console.log(
          'DEBUG Stats - count by string:',
          countByString,
          'count by ObjectId:',
          countByObjectId,
        );
        ...
        let postStats = await feedPostsCollection
          .aggregate([
            { $match: { userId: id } },
            {
              $group: {
        ...
        if (postStats.length === 0) {
          postStats = await feedPostsCollection
            .aggregate([
  ...
  router.get('/:id/activity', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const reactedPosts = await feedPostsCollection.find({ _id: { $in: postIds } }).toArray();
      ...
      try {
        ...
        const commentedPosts = await feedPostsCollection.find({ _id: { $in: postIds } }).toArray();
      ...
      res.send({
        ...
        hasMore: activities.length > parseInt(skip, 10) + parseInt(limit, 10),
      });
    } catch (error) {
  ...
  router.get('/homie-status/:targetId', auth, async (req, res) => {
    ...
    try {
      ...
      const receivedRequest = currentUser.homieRequests?.received?.find((r) => r.from === targetId);
      if (receivedRequest) {
