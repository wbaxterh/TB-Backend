...
module.exports = (db) => {
  ...
  const updateSpotRating = async (spotId) => {
    const reviews = await reviewsCollection.find({ spotId: spotId, status: 'active' }).toArray();
  ...
  router.put('/:reviewId', auth, async (req, res) => {
    ...
    try {
      ...
      await reviewsCollection.updateOne({ _id: new ObjectId(reviewId) }, { $set: updateFields });
