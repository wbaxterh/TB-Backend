...
const DEFAULT_SPOTS = [
  { name: "Old State House Steps", city: "Hartford", state: "CT", lat: 41.7637, lng: -72.6735, description: "Classic marble steps, multiple ledge options", category: "home" },
  { name: "Bushnell Park Rails", city: "Hartford", state: "CT", lat: 41.7627, lng: -72.6823, description: "Park rails near the carousel, mellow vibes", category: "home" },
  { name: "Yale Gothic Arches", city: "New Haven", state: "CT", lat: 41.3111, lng: -72.9267, description: "Stone arches with perfect wallride angles", category: "home" },
  { name: "Sterling Library Benches", city: "New Haven", state: "CT", lat: 41.3116, lng: -72.9289, description: "Granite benches, smooth ground", category: "home" },
  { name: "Frozen Seaport Docks", city: "Mystic", state: "CT", lat: 41.3615, lng: -71.9662, description: "Wooden dock edges, waterfront backdrop", category: "home" },
  { name: "Old Crane Arms", city: "Mystic", state: "CT", lat: 41.3601, lng: -71.9658, description: "Industrial crane structures, unique features", category: "home" },
  { name: "Windham Textile Mill", city: "Windham", state: "CT", lat: 41.7098, lng: -72.1573, description: "Brick ruins, loading ramps, raw industrial", category: "home" },
  { name: "Brass Factory Row", city: "Bridgeport", state: "CT", lat: 41.1792, lng: -73.1894, description: "Loading bays, brick ledges, gritty aesthetic", category: "home" },
  { name: "Oyster Dock Pilings", city: "Norwalk", state: "CT", lat: 41.0968, lng: -73.4154, description: "Dock pilings and flat ground by the water", category: "home" },
  { name: "Boston (Weekend Trip)", city: "Boston", state: "MA", lat: 42.3601, lng: -71.0589, description: "Weekend AirBnb trip — multiple spots TBD", category: "expansion" },
  { name: "Worcester (Weekend Trip)", city: "Worcester", state: "MA", lat: 42.2626, lng: -71.8023, description: "Weekend AirBnb trip — industrial city, tons of street spots", category: "expansion" },
  { name: "Portland ME (Weekend Trip)", city: "Portland", state: "ME", lat: 43.6591, lng: -70.2568, description: "Weekend AirBnb trip — waterfront + Old Port", category: "expansion" },
  { name: "Burlington VT (Weekend Trip)", city: "Burlington", state: "VT", lat: 44.4759, lng: -73.2121, description: "Weekend AirBnb trip — Church St + waterfront", category: "expansion" },
];
...
router.get("/spots", async (req, res) => {
  try {
    ...
    if (spots.length === 0) {
      await db.collection("ck_spots").insertMany(DEFAULT_SPOTS.map(s => ({ ...s, createdAt: new Date() })));
  ...
  try {
    ...
    if (!spot.lat && spot.city) {
      try {
        ...
        const geo = await axios.get(`https://geocoding-api.open-meteo.com/v1/search?name=${q}&count=1&language=en&format=json`);
        if (geo.data.results && geo.data.results.length > 0) {
          spot.lat = geo.data.results[0].latitude;
          spot.lng = geo.data.results[0].longitude;
        }
      } catch(e) { /* geocode failed, skip */ }
    }
...
router.post("/spots/:id/photos", async (req, res) => {
  try {
    ...
    await db.collection("ck_spots").updateOne(
...
router.post("/trips/:id/join", async (req, res) => {
  try {
    ...
    const { name } = req.body;
    if (!name) return res.status(400).json({ error: "name required" });
    await db.collection("ck_trips").updateOne(
      { _id: new ObjectId(req.params.id) },
