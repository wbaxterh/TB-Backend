...
module.exports = (_db) => {
  ...
  // ConnectaKids uses a DIFFERENT database (myFirstDatabase), not TrickList2
  const ckDb = getClient().db('myFirstDatabase');
  ...
  router.get('/spots', async (_req, res) => {
    try {
      const spots = await ckDb.collection('ck_spots').find({}).toArray();
      if (spots.length === 0) {
        await ckDb
          .collection('ck_spots')
          .insertMany(DEFAULT_SPOTS.map((s) => ({ ...s, createdAt: new Date() })));
        const seeded = await ckDb.collection('ck_spots').find({}).toArray();
  ...
  router.post('/spots', async (req, res) => {
    try {
      ...
      const result = await ckDb.collection('ck_spots').insertOne(spot);
  ...
  router.delete('/spots/:id/photos/:index', async (req, res) => {
    try {
      const spot = await ckDb.collection('ck_spots').findOne({ _id: new ObjectId(req.params.id) });
  ...
  router.delete('/spots/:id', async (req, res) => {
    try {
      await ckDb.collection('ck_spots').deleteOne({ _id: new ObjectId(req.params.id) });
      res.json({ deleted: true });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  router.get('/availability', async (req, res) => {
    try {
      ...
      const avail = await ckDb.collection('ck_availability').find(filter).toArray();
      res.json(avail);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  router.post('/availability', async (req, res) => {
    try {
      ...
      const existing = await ckDb.collection('ck_availability').findOne({ date, nameLower });
      if (existing) {
        await ckDb.collection('ck_availability').deleteOne({ _id: existing._id });
        return res.json({ removed: true, date });
      }
      const entry = { date, name: name.trim(), nameLower, createdAt: new Date() };
      await ckDb.collection('ck_availability').insertOne(entry);
      res.json(entry);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  router.get('/trips', async (_req, res) => {
    try {
      const trips = await ckDb.collection('ck_trips').find({}).sort({ date: 1 }).toArray();
      res.json(trips);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  router.post('/trips', async (req, res) => {
    try {
      ...
      delete trip.name;
      const result = await ckDb.collection('ck_trips').insertOne(trip);
  ...
  router.delete('/trips/:id', async (req, res) => {
    try {
      await ckDb.collection('ck_trips').deleteOne({ _id: new ObjectId(req.params.id) });
      res.json({ deleted: true });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
