...
function buildServer(db) {
  const server = new Server(
    { name: 'trickbook', title: 'TrickBook Action Sports', version: '1.0.0' },
    {
      capabilities: { tools: {} },
      instructions:
        'Use TrickBook for current action-sports spots, tricks, events, films, and culture. Preserve source_url links in answers.',
    },
  );
...
module.exports = (db) => {
  ...
  router.get('/', (_req, res) =>
    res.status(405).set('Allow', 'POST').json({ error: 'Use POST for MCP Streamable HTTP' }),
  );
  router.delete('/', (_req, res) =>
    res.status(405).set('Allow', 'POST').json({ error: 'Stateless MCP server' }),
  );
  return router;
};
